...
class Excel
{
	...
	public function __construct($param = array())
	{
		...
		// 出错跳转地址
		$this->jump_url = empty($param['jump_url']) ? '' : $param['jump_url'];
	...
	public function Export()
	{
		...
		{
			isset($v['width']) ?  $sheet->getColumnDimension($v['col'])->setWidth($v['width']) : $sheet->getColumnDimension($v['col'])->setAutoSize(true);    			
			$sheet->setCellValue($v['col'].'1', $v['name']);
			$sheet->getStyle($v['col'].'1')->getFont()->setBold(true);  //加粗
		}
		...
		foreach($this->data as $k=>$v)
		{
			...
			if(is_array($v) && !empty($v))
			{
				...
				{
					isset($v[$tk]) || $v[$tk]='';
					in_array($tk, ['addtime','edittime']) && $v[$tk] = format_datetime($v[$tk]);
					$sheet->setCellValueExplicit($tv['col'].$i, $v[$tk], PHPExcel_Cell_DataType::TYPE_STRING);
				}
			}
		}
